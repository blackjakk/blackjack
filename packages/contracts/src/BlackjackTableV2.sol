...
///           UNRESERVED funds stays treasury-gated — groundwork for LP shares later).
...
contract BlackjackTableV2 is IRandomnessConsumer, AccessControl, ReentrancyGuard, Pausable {
    ...
    function double(uint256 gameId) external nonReentrant whenNotPaused {
        Game storage g = _ownedGame(gameId);
        if (g.state != GameState.PLAYER_TURN) revert InvalidState(gameId, g.state);
        if (g.playerCount != 2 || g.doubled || !_doubleAllowed(g)) revert CannotDouble(gameId);
    ...
    function surrender(uint256 gameId) external nonReentrant {
        Game storage g = _ownedGame(gameId);
        if (g.state != GameState.PLAYER_TURN) revert InvalidState(gameId, g.state);
        if (!_lateSurrender || g.playerCount != 2 || g.doubled) revert CannotSurrender(gameId);
